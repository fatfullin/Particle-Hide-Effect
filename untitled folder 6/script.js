...
function createParticleEffect(targetElement, options = {}) {
    ...
    class Particle {
        ...
        reset(initialize = true) {
            ...
            // Определяем поведение частицы (может вылетать или нет)
            if (settings.useZones) {
                ...
                // По умолчанию начинаем в центральной зоне
                ...
                // Позиция в центральной зоне
                this.x = centerX + Math.random() * centerWidth;
                this.y = centerY + Math.random() * centerHeight;
            } else {
                // Стандартная позиция (по всей области)
